<?php
	...
	Class fieldPassword extends Field {
		...
		function displayPublishPanel(&$wrapper, $data=NULL, $flagWithError=NULL, $fieldnamePrefix=NULL, $fieldnamePostfix=NULL){
			...
			$label->appendChild(Widget::Input('fields'.$fieldnamePrefix.'['.$this->get('element_name').']'.$fieldnamePostfix, (strlen($value) != 0 ? $value : NULL), 'password'));
		...
		public function prepareTableValue($data, XMLElement $link=NULL){
			...
			for ($i=1; $i <= ($password_length <= 12 ? $password_length : 12); $i++) {
					$obscured_password .= $password_obscure_char;
			}
			
			$value = $obscured_password . ' (' . $password_length . ' characters)';
